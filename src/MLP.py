...
class MLP(object):
    ...
    def finalizeMLP(self):
        ...
        # print("Construct diagram for each monomial")
    ...
    """
    Forms and solves the root node LP relaxation for a given linearization using Gurobi
    """

    def solveLP(self, lin=None, print_flag=0, time_limit=60, num_threads=1):

        lp = gb.Model()

        lp.setParam("OutputFlag", print_flag)
        lp.setParam("TimeLimit", time_limit)
        ...
        y = lp.addVars(ylist, lb=self.all_var_lb, ub=self.all_var_ub, obj=self.all_obj_coeff, name="y")
        ...
        # McCormick envelopes
        for triple in lin:
            self.addMcCormick(triple[0], triple[1], triple[2], y, lp)

        # add the constraints
        for ic in range(self.ncon):
            con_lhs = gb.LinExpr(0.0)
            for mon in self.cons_coeff[ic].keys():
                con_lhs += self.cons_coeff[ic][mon] * y[mon]
            lp.addConstr(con_lhs <= self.cons_ub[ic])

        lp.update()
        lp.optimize()

        y_sol = {mon: y[mon].getAttr("x") for mon in ylist}

        results = ut.gurobiResults(lp, 0)
        results.y_sol = y_sol

        if self.obj_sense == "Max":
            results.obj = -1.0 * results.obj
        results.obj = results.obj + self.obj_offset

        results.objbnd = lp.ObjBound
        results.cpu = lp.Runtime
    ...
    def solveLPscipy(self, lin=None, print_flag=0):
        ...
        for mon in self.coeff.keys():
            c[ylist.index(mon)] = self.coeff[mon]
        ...
        self.lp_results = ut.struct()
        self.lp_results.nvar = n_ylist
        self.lp_results.ncon = 3 * n_arcs
        self.lp_results.obj = res.fun
        self.lp_results.cput = 0.0

        if self.obj_sense == "Max":
            self.lp_results.obj = -1.0 * self.lp_results.obj
        self.lp_results.obj = self.lp_results.obj + self.obj_offset

    """
    Forms and solves the dual of the root node dual LP relaxation for a given linearization
        lin - choice of linearization, if None then use all arcs
        print_flag - controls Gurobi output
    """

    def solveDualLP(self, lin=None, print_flag=0):

        if not (self.zerone_bdns):
            raise Exception("Dual LP is only implemented for variables with 0-1 bounds")
        ...
        mu = {}
        for mon in ylist:
            mu.update({mon: lp.addVar(lb=0.0, obj=0.0, name="mu" + str(mon))})
        lam1 = lp.addVars(triples, lb=0.0, obj=0.0, name="lam1")
        lam2 = lp.addVars(triples, lb=0.0, obj=0.0, name="lam2")
        lam3 = lp.addVars(triples, lb=0.0, obj=0.0, name="lam3")

        # constraint multipliers
        cons_lam = {ic: lp.addVar(lb=0.0, obj=0.0, name="con_lam") for ic in range(self.ncon)}
        ...
        for mon in ylist:
            ...
            expr += mu[mon] + self.all_obj_coeff[mon]

            for ic in range(self.ncon):
                for kmon in self.cons_coeff[ic].keys():
                    if mon == kmon:
                        expr += self.cons_coeff[ic][mon] * cons_lam[ic]
        ...
        for mon in ylist:
            obj += -mu[mon]
        for t in triples:
            if lin is None or t in lin or (t[1], t[0], t[2]) in lin:
                obj += -lam3[t]
            else:
                lp.addConstr(lam1[t] + lam2[t] + lam3[t] == 0)
        for ic in range(self.ncon):
            obj += -self.cons_ub[ic] * cons_lam[ic]

        lp.setObjective(obj, gb.GRB.MAXIMIZE)
        lp.update()
        lp.optimize()

        mu_sol = {mon: mu[mon].getAttr("x") for mon in ylist}
        lam1_sol = {t: lam1[t].getAttr("x") for t in triples}
        lam2_sol = {t: lam2[t].getAttr("x") for t in triples}
        lam3_sol = {t: lam3[t].getAttr("x") for t in triples}

        results = ut.gurobiResults(lp, 0)
        results.mu_sol = mu_sol
        results.lam1_sol = lam1_sol
        results.lam2_sol = lam2_sol
        results.lam3_sol = lam3_sol
    ...
    # Single function to simplify script calls
    def Linearization(self, lin_type):
        if lin_type == "full":
            return self.fullLinearization()
        else:
            return self.heuristicLinearization(lin_type)
